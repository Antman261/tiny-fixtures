...
var createRefGetter = function (rows, index) {
    return function (key) {
        return function () { return rows[index][key]; };
    };
};
/**
 *
...
var tinyFixtures = function (pool) {
    var createFixtures = function (table, rows, primaryKeyName) {
        var rowsEnhanced = [];
        rows.forEach(function (r, index) { return rowsEnhanced.push(__assign(__assign({}, r), { getRefByKey: createRefGetter(rowsEnhanced, index) })); });
        var primaryKeys = [];
        var pkName;
        var setupFixtures = function () { return __awaiter(void 0, void 0, void 0, function () {
            var rowsResolved, mapRowToInsertQuery, pendingInsertQueries, results, mixedArr;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        rowsResolved = rows.map(function (row) {
                            var _a;
                            var unresolvedKey = Object
                                .keys(row)
                                // @ts-ignore
                                .find(function (k) { return typeof row[k] === 'function'; });
                            if (unresolvedKey) {
                                // @ts-ignore
                                var resolvedValue = row[unresolvedKey]();
                                return __assign(__assign({}, row), (_a = {}, _a[unresolvedKey] = resolvedValue, _a));
                            }
                            return row;
                        });
                        mapRowToInsertQuery = query_1.createRowToQueryMapper(table, pool);
                        pendingInsertQueries = rowsResolved.map(mapRowToInsertQuery);
                        return [4 /*yield*/, Promise.all(__spreadArray([], pendingInsertQueries))];
                    case 1:
                        results = _a.sent();
